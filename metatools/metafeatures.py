...
class metafeatures:
    \"\"\"
    Essa classe contém métodos para a criação de metafeatures de uma base de dados. 
    Parameters:
    ----------------------
    
        dataset_info : tuple os three values. (dataset_name <str>, dataset <pandas.DataFrame>, target_name = <str>)
    \"\"\"
""" 
...
def get_class_entropy(dataset, target_name):
    ...
    for value in dataset[target_name].unique():
        ...
        #print('Value: {};\t Count: {};\t class_freq: {}'.format(value, target_frequency[value], class_freq))
    return entropy
